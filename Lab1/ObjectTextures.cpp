...
Textures::Textures(const std::string& fileName)
{
	...
	glGenTextures(1, &handler); // number of and address of textures
	glBindTexture(GL_TEXTURE_2D, handler); //bind texture - define type 
...
Textures::~Textures()
{
	glDeleteTextures(1, &handler); // number of and address of textures
}
...
void Textures::Bind(unsigned int unit)
{
	...
	glBindTexture(GL_TEXTURE_2D, handler); //type of and texture to bind to unit
}
