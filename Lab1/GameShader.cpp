...
GameShader::GameShader(const std::string& filename)
{
	...
	shaders[0] = CreateShader(LoadShader("C:\\Users\\Jack\\Desktop\\JackMcEleneyCoursework\\res\\shader.vert"), GL_VERTEX_SHADER); // create vertex shader
	shaders[1] = CreateShader(LoadShader("C:\\Users\\Jack\\Desktop\\JackMcEleneyCoursework\\res\\shader.frag"), GL_FRAGMENT_SHADER); // create fragment shader
	...
	uniforms[UniformTransform] = glGetUniformLocation(program, "transform"); // associate with the location of uniform variable within a program
}
...
void GameShader::Update(const Position& position, const Cam& camera)
{
	glm::mat4 mvp = camera.GetViewProjection() * position.GetModel();
	glUniformMatrix4fv(uniforms[UniformTransform], 1, GLU_FALSE, &mvp[0][0]);
}
...
std::string GameShader::LoadShader(const std::string& fileName)
{
	...
	{
		std::cerr << "Unable to load shader: " << fileName << std::endl; //error message if shader can't load
	}
